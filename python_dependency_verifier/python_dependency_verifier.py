...
def get_append_list(file_contents,ignore_this):
    ...
    while "install_requires.append(" in file_contents:
        ...
        list_of_appended_requirements.append(string_dependency_to_dict(file_contents[:ends_at].replace(" ", "").replace("\'","").replace("\"","")))
    return list_of_appended_requirements


def get_install_requires_field_contents(file_contents):
    file_contents = remove_comments(file_contents).replace(" = ", "=")
    ...
    list_of_requirements = file_contents.replace(" ", "").replace("\'","").replace("\"","").split(",")
    return list_of_requirements

def filter_list_for_unversioned_dependencies(list_of_dependencies, ignore_this):
    unversioned_dependencies=[]
    for dependency in list_of_dependencies:
        if dependency!='':
            if "==" not in dependency:
                if not re.match(re.compile(ignore_this), dependency):
                    unversioned_dependencies.append(string_dependency_to_dict(dependency))
...
def filter_list_for_versioned_dependencies(list_of_dependencies, ignore_this):
    ...
    for dependency in list_of_dependencies:
        if dependency!='':
            if "=" in dependency:
                if not re.match(re.compile(ignore_this), dependency):
                    versioned_dependencies.append(string_dependency_to_dict(dependency))
...
def string_dependency_to_dict(string_dependency):
    ...
    dependency_dict = [dependency_dict[0], is_locked, version_relation, get_latest_version_number(dependency_dict[0])]
...
def check_all_filename_in_subdirs(path,filename_to_search_for, field_dependency_name, ignore_this):
    for dir in os.listdir(path):
        try:
            filename = path+dir+filename_to_search_for
            file_contents = get_file_contents(filename)
            file_contents = file_contents.replace(" = ", "=")
            if not field_dependency_name in file_contents: continue
            dependency_list = get_dependency_with_latest_version(
                get_install_requires_field_contents(file_contents), ignore_this)
